...
2. Установите библиотеку прямо из GitHub:
```bash
...
Клонировать репозиторий вручную не нужно — `pip` сам скачает и установит библиотеку и её зависимости (в том числе pygame) в ваше окружение.

## Структура проекта

Рекомендуемая структура для вашей игры:
...
## Разработка самой библиотеки

Если вы клонировали этот репозиторий, чтобы дорабатывать саму библиотеку (а не игру на её основе), установите её в режиме редактирования — так изменения в коде `pygine` подхватываются сразу, без переустановки:
```bash
git clone https://github.com/dnbbch/pygine.git
cd pygine
python -m venv venv
...
Примеры из папки `examples/` рассчитаны на такую editable-установку и запускаются как обычные Python-скрипты из любой директории:
```bash
python examples/example_all_animations.py
```
